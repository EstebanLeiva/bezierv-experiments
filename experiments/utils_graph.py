...
def get_sample(values, sample_size, rng=None):
  if not values:
    return []
  ...
  sample = random.sample(values, k=sample_size, random=rng)
  return sample
...
def fit_graph(data, 
    ...
    with open(pkl_times, 'wb') as f:
        pickle.dump(times, f)
    pkl_failures = 'data/fitted_failures_' + method + '.pkl'
    with open(pkl_failures, 'wb') as f:
        pickle.dump(num_failures, f)

    return graph, errors, times, num_failures
